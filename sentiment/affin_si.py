...
class Afinn(object):
    ...
    True
    >>> afinn.score('අකටයුතු') < 0
    True
    """

    def __init__(self, language=prompt_language, emoticons=False, word_boundary=True):
    ...
    def read_word_file(filename):
        ...
        with codecs.open(filename, encoding='UTF-8') as fid:
            for n, line in enumerate(fid):
                try:
                    word, score = line.strip().split(split_char)
                except ValueError:
    ...
    def find_all(self, text, clean_whitespace=True):
        ...
        print("pattern: ", re.finditer(self._pattern, "අත්පොළසන් නොකල"))
    ...
    def scores_with_pattern(self, text):
        ...
        --------
        >>> afinn = Afinn()
        >>> afinn.scores_with_pattern('අතපසුවිමක් අත්පොළසන්')
        [3, -3]
...
affinity = Afinn()
print(affinity.score('This is really bad.'))
print(affinity.score('අතපසුවිමක් අත්පොළසන්'))